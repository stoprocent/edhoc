...
vec cbor_unsigned_with_type( uint8_t type, int i ) {
    ...
    if ( i < 24 )
        ...
        return { (uint8_t) (type | 0x1A), (uint8_t)((i) >> 24), (uint8_t)((i & 0xff0000) >> 16), (uint8_t)((i & 0xff00) >> 8), (uint8_t) (i & 0xff) };
}
...
void test_vectors( EDHOCKeyType type_I, COSECred credtype_I, COSEHeader attr_I,
    ...
    // G_Z
    vec seed_x = random_vector( crypto_kx_SEEDBYTES );
    vec seed_y = random_vector( crypto_kx_SEEDBYTES );
    
    // PRKs
    seed_x = vec { 0x36, 0x8E, 0xC1, 0xF6, 0x9A, 0xEB, 0x65, 0x9B, 0xA3, 0x7D, 0x5A, 0x8D, 0x45, 0xB2, 0x1B, 0xDC, 0x02, 0x99, 0xDC, 0xEA, 0xA8, 0xEF, 0x23, 0x5F, 0x3C, 0xA4, 0x2C, 0xE3, 0x53, 0x0F, 0x95, 0x25 };
    seed_y = vec { 0xE2, 0xF4, 0x12, 0x67, 0x77, 0x20, 0x5E, 0x85, 0x3B, 0x43, 0x7D, 0x6E, 0xAC, 0xA1, 0xE1, 0xF7, 0x53, 0xCD, 0xCC, 0x3E, 0x2C, 0x69, 0xFA, 0x88, 0x4B, 0x0A, 0x1A, 0x64, 0x09, 0x77, 0xE4, 0x18 };
    
    auto [ X, G_X ] = ecdh_key_pair(seed_x);
    auto [ Y, G_Y ] = ecdh_key_pair(seed_y);
    
    vec G_XY = shared_secret( X, G_Y );

//    vec seed_r = random_vector( crypto_kx_SEEDBYTES );
//    vec seed_i = random_vector( crypto_kx_SEEDBYTES );
//
//    auto [ R, G_R ] = ecdh_key_pair(seed_r);
//    auto [ I, G_I ] = ecdh_key_pair(seed_i);
//
    // Responder Keys
    vec R = vec{ 0xEF, 0x14, 0x0F, 0xF9, 0x00, 0xB0, 0xAB, 0x03, 0xF0, 0xC0, 0x8D, 0x87, 0x9C, 0xBB, 0xD4, 0xB3, 0x1E, 0xA7, 0x1E, 0x6E, 0x7E, 0xE7, 0xFF, 0xCB, 0x7E, 0x79, 0x55, 0x77, 0x7A, 0x33, 0x27, 0x99 };
    vec G_R = vec{ 0xA1, 0xDB, 0x47, 0xB9, 0x51, 0x84, 0x85, 0x4A, 0xD1, 0x2A, 0x0C, 0x1A, 0x35, 0x4E, 0x41, 0x8A, 0xAC, 0xE3, 0x3A, 0xA0, 0xF2, 0xC6, 0x62, 0xC0, 0x0B, 0x3A, 0xC5, 0x5D, 0xE9, 0x2F, 0x93, 0x59};

    // Initiator Keys
    vec I = vec{  0x4C, 0x5B, 0x25, 0x87, 0x8F, 0x50, 0x7C, 0x6B, 0x9D, 0xAE, 0x68, 0xFB, 0xD4, 0xFD, 0x3F, 0xF9, 0x97, 0x53, 0x3D, 0xB0, 0xAF, 0x00, 0xB2, 0x5D, 0x32, 0x4E, 0xA2, 0x8E, 0x6C, 0x21, 0x3B, 0xC8 };
    vec G_I = vec{ 0xED, 0x06, 0xA8, 0xAE, 0x61, 0xA8, 0x29, 0xBA, 0x5F, 0xA5, 0x45, 0x25, 0xC9, 0xD0, 0x7F, 0x48, 0xDD, 0x44, 0xA3, 0x02, 0xF4, 0x3E, 0x0F, 0x23, 0xD8, 0xCC, 0x20, 0xB7, 0x30, 0x85, 0x14, 0x1E };
    
    vec G_RX = shared_secret( R, G_X );
    vec G_IY = shared_secret( I, G_Y );
    
//    vec seed_sk_r = random_vector( crypto_sign_SEEDBYTES );
//    vec seed_sk_i = random_vector( crypto_sign_SEEDBYTES );
//
//    auto [ SK_R, PK_R ] = sign_key_pair(seed_sk_r);
//    auto [ SK_I, PK_I ] = sign_key_pair(seed_sk_i);

    // Responder Keys
    vec SK_R = vec{ 0xEF, 0x14, 0x0F, 0xF9, 0x00, 0xB0, 0xAB, 0x03, 0xF0, 0xC0, 0x8D, 0x87, 0x9C, 0xBB, 0xD4, 0xB3, 0x1E, 0xA7, 0x1E, 0x6E, 0x7E, 0xE7, 0xFF, 0xCB, 0x7E, 0x79, 0x55, 0x77, 0x7A, 0x33, 0x27, 0x99 };
    vec PK_R = vec{ 0xA1, 0xDB, 0x47, 0xB9, 0x51, 0x84, 0x85, 0x4A, 0xD1, 0x2A, 0x0C, 0x1A, 0x35, 0x4E, 0x41, 0x8A, 0xAC, 0xE3, 0x3A, 0xA0, 0xF2, 0xC6, 0x62, 0xC0, 0x0B, 0x3A, 0xC5, 0x5D, 0xE9, 0x2F, 0x93, 0x59};

    // Initiator Keys
    vec SK_I = vec{  0x4C, 0x5B, 0x25, 0x87, 0x8F, 0x50, 0x7C, 0x6B, 0x9D, 0xAE, 0x68, 0xFB, 0xD4, 0xFD, 0x3F, 0xF9, 0x97, 0x53, 0x3D, 0xB0, 0xAF, 0x00, 0xB2, 0x5D, 0x32, 0x4E, 0xA2, 0x8E, 0x6C, 0x21, 0x3B, 0xC8 };
    vec PK_I = vec{ 0xED, 0x06, 0xA8, 0xAE, 0x61, 0xA8, 0x29, 0xBA, 0x5F, 0xA5, 0x45, 0x25, 0xC9, 0xD0, 0x7F, 0x48, 0xDD, 0x44, 0xA3, 0x02, 0xF4, 0x3E, 0x0F, 0x23, 0xD8, 0xCC, 0x20, 0xB7, 0x30, 0x85, 0x14, 0x1E };
    ...
    vec cred_r = vec{  };
    
    // Initiator Certificate
    vec cred_i = vec{  };
...
int main( void ) {
    ...
    test_vectors( sig, cred_x509, x5t, sig, cred_x509, x5t, 0, 37400 );
//    /test_vectors( sig, cred_x509, x5t, sig, cred_x509, x5t, 0, 37400, false, false );
